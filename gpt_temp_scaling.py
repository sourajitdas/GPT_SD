...
class Block(nn.Module):
    ...
    def forward(self, x):
        # Perform the skip connections for better optimization
...
class GPTLanguageModel(nn.Module):
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0):
        ...
        for _ in range(max_new_tokens):
            ...
            logits = logits / temperature  # Scale logits before softmax
...
generated_text_low_temp = decode(m.generate(context, max_new_tokens=500, temperature=0.7)[0].tolist())
generated_text_high_temp = decode(m.generate(context, max_new_tokens=500, temperature=1.5)[0].tolist())

# Save results to files
with open('low_temp.txt', 'w', encoding='utf-8') as f:
    f.write(generated_text_low_temp)

with open('high_temp.txt', 'w', encoding='utf-8') as f:
    f.write(generated_text_high_temp)

# BLEU Score Calculation (Optional)
reference_text = text[:1000]  # Use the first 1000 characters as reference
generated_text_sample = generated_text_low_temp[:1000]  # Compare same length
